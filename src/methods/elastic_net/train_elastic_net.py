...
# Se crea el directorio para almacenar los resultados de Elastic Net
model_dir = Path("src") / "methods" / "elastic_net"
...
# Se instancia y entrena el modelo Elastic Net
print("Entrenando el modelo Elastic Net...")
modelo = ElasticNet(
    alpha=0.1,       
    l1_ratio=0.5,    
    random_state=None,
...
if VIEW_GRAPH:
    ...
    if SAVE_GRAPH:
        plt.savefig(results_dir / f'en_prediction_Ftemp({ADD_FEATURES_TEMPORALES})_Flag({ADD_FEATURES_LAG}).png')
    else:
        plt.show()

    # Se genera el gráfico de coeficientes (Importancia en modelos lineales)
    if hasattr(modelo, "coef_"):
        ...
        # Se grafican los valores reales de los coeficientes (positivos o negativos)
        ...
        if SAVE_GRAPH:
            plt.savefig(results_dir / f'en_coefficients_Ftemp({ADD_FEATURES_TEMPORALES})_Flag({ADD_FEATURES_LAG}).png')
        else:
            plt.show()