...
# GPR es (O(N^3)). Limitamos el entrenamiento.
...
results_dir = model_dir / f"results_{nombre_dataset}_gpr"
...
# Separar los datos
X_train_raw = df_train[features] # Renombrado a _raw
y_train = df_train[Y_COLUMN]
X_test_raw = df_test[features]   # Renombrado a _raw
...
# Reducción de datos para GPR
if len(X_train) > MAX_TRAIN_SAMPLES:
    print(f"⚠️ ADVERTENCIA: {len(X_train)} datos son demasiados para GPR estándar.")
    print(f"   Submuestreando a los últimos {MAX_TRAIN_SAMPLES} puntos para entrenamiento.")
    X_train = X_train[-MAX_TRAIN_SAMPLES:]
    y_train = y_train.iloc[-MAX_TRAIN_SAMPLES:]
    
# Entrenar el modelo
print("Entrenando el modelo Gaussian Process Regressor con kernel RBF...")
kernel = C(1.0, (1e-3, 1e3)) * RBF(length_scale=1.0, length_scale_bounds=(1e-2, 1e2)) + \
         WhiteKernel(noise_level=1, noise_level_bounds=(1e-10, 1e+1))

modelo = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=2, normalize_y=True, random_state=None)

modelo.fit(X_train, y_train)
print(f"Modelo entrenado. Kernel optimizado: {modelo.kernel_}")
...
if VIEW_GRAPH:
    ...
    plt.plot(df_test.index, y_pred, label='Predicción GPR (Media)', color='red', alpha=0.8, linewidth=1.5)
    ...
    if VENTANA_DE_PREDICCION > 0:
        titulo_prediccion = f'Predicción de {TARGET_COLUMN} ({VENTANA_DE_PREDICCION} min. a futuro) usando GPR'
    else:
        titulo_prediccion = f'Predicción de {TARGET_COLUMN} (Actual) usando GPR'

    plt.title(titulo_prediccion)
    plt.legend(loc='upper left', title=f'Prediciendo t+{VENTANA_DE_PREDICCION} min\nRMSE={rmse:.3f}\nR2={r2:.3f}\nMAE={mae:.3f}')
    
    if SAVE_GRAPH:
        plt.savefig(results_dir / f'gpr_prediction_ventana({VENTANA_DE_PREDICCION})_Ftemp({ADD_FEATURES_TEMPORALES}).png')
    else:
        plt.show()