...
# Se crea el directorio para almacenar los resultados del Árbol de Decisión
model_dir = Path("src") / "methods" / "decision_tree"
...
# Se instancia y entrena el modelo de Árbol de Decisión
print("Entrenando el modelo Decision Tree Regressor...")
modelo = DecisionTreeRegressor(
    max_depth=20,            # Permite un poco más de profundidad que un modelo muy básico
    min_samples_leaf=50,     # Reduce el tamaño de los "escalones" al permitir hojas más pequeñas
    min_samples_split=30,    # Facilita que el árbol encuentre más patrones locales
    max_features=None,       # Considera todas las variables para asegurar mejores cortes
    random_state=None
)
modelo.fit(X_train, y_train)
print("Modelo entrenado con éxito.")

# Se realizan las predicciones y se calculan las métricas de error
...
if VIEW_GRAPH:
    ...
    if SAVE_GRAPH:
        plt.savefig(results_dir / f'dt_prediction_Ftemp({ADD_FEATURES_TEMPORALES})_Flag({ADD_FEATURES_LAG}).png')
    else:
        plt.show()

    # Se genera el gráfico de importancia de variables si el modelo lo permite
    if hasattr(modelo, "feature_importances_"):
        ...
        plt.title("Importancia de las Features (Decision Tree)")
        plt.bar(range(len(importances)), importances[indices], align="center")
        plt.xticks(range(len(importances)), labels, rotation=90)
        plt.tight_layout()
        
        if SAVE_GRAPH:
            plt.savefig(results_dir / f'dt_feature_importance_Ftemp({ADD_FEATURES_TEMPORALES})_Flag({ADD_FEATURES_LAG}).png')
        else:
            plt.show()