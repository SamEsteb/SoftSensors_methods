...
# Se crea el directorio para almacenar los resultados de MLR
model_dir = Path("src") / "methods" / "mlr"
...
# Se inicializa y ajusta el escalador
# Nota: En MLR simple, el escalado no afecta la predicción final (R2), 
# pero es crucial para interpretar los coeficientes correctamente.
print("Estandarizando las características...")
scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train)
X_test_scaled = scaler.transform(X_test)

# Se instancia y entrena el modelo de Regresión Lineal (MLR)
print("Entrenando el modelo Linear Regression (MLR)...")
modelo = LinearRegression(
    n_jobs=-1  # Usa todos los procesadores para acelerar el cálculo matricial
)
...
if VIEW_GRAPH:
    ...
    tipo_mdo = "MLR (Linear Regression)"
    ...
    if SAVE_GRAPH:
        plt.savefig(results_dir / f'mlr_prediction_Ftemp({ADD_FEATURES_TEMPORALES})_Flag({ADD_FEATURES_LAG}).png')
    else:
        plt.show()
    ...
    if hasattr(modelo, "coef_"):
        ...
        plt.title("Coeficientes del Modelo (MLR)")
        ...
        if SAVE_GRAPH:
            plt.savefig(results_dir / f'mlr_coefficients_Ftemp({ADD_FEATURES_TEMPORALES})_Flag({ADD_FEATURES_LAG}).png')
        else:
            plt.show()