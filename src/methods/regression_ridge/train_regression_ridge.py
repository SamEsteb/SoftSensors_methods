...
# Se crea el directorio para almacenar los resultados de Ridge
model_dir = Path("src") / "methods" / "regression_ridge"
...
# Se instancia y entrena el modelo Ridge con parámetros fijos
print("Entrenando el modelo Ridge...")
modelo = Ridge(
    alpha=1.0,        
    random_state=None
)
...
if VIEW_GRAPH:
    ...
    if SAVE_GRAPH:
        plt.savefig(results_dir / f'ridge_simple_prediction_Ftemp({ADD_FEATURES_TEMPORALES})_Flag({ADD_FEATURES_LAG}).png')
    else:
        plt.show()
    ...
    if hasattr(modelo, "coef_"):
        ...
        plt.title(f"Coeficientes del Modelo (Ridge alpha={modelo.alpha})")
        ...
        if SAVE_GRAPH:
            plt.savefig(results_dir / f'ridge_simple_coefficients_Ftemp({ADD_FEATURES_TEMPORALES})_Flag({ADD_FEATURES_LAG}).png')
        else:
            plt.show()